...
class Regression:
    ...
    def phylo(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # PGLS-derived correlation: derived from the slope's t-statistic, so
            # sign(r) matches sign(slope) and the legend r is consistent with
            # the regression line drawn from the PGLS fit.
            tval <- s$tTable["Ne","t-value"]
            df_resid <- s$dims$N - s$dims$p
            r <- sign(tval) * sqrt(tval^2 / (tval^2 + df_resid))
