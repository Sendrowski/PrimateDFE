...
try:
    ...
    dfe_file = "results/dfe/catarrhini/dfe.unfolded.8.discrete.del.noeps.csv"
...
# Load DFEs, collapse subspecies → species, discretize + average
# ----------------------------
# Deleterious-only models have no mass at S >= 0; omit that empty bin.
if sub_model == "del":
    intervals = np.array([-np.inf, -10, -1, 0])
    interval_labels = ["(-inf, -10)", "[-10, -1)", "[-1, 0)"]
else:
    intervals = np.array([-np.inf, -10, -1, 0, np.inf])
    interval_labels = ["(-inf, -10)", "[-10, -1)", "[-1, 0)", "[0, inf)"]
