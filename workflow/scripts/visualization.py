...
class DFEvsNePlotter:
    ...
    def plot_two_datasets_stacked(
        ...
        ``color_by="dataset"`` colours every point by its dataset (default).
        ``color_by="taxon"`` colours points by taxonomic group (as in the
        single-dataset figures) and distinguishes the datasets by marker shape
        (``markers``) and regression line style (``linestyles``).
        """
        if fig is None:
            fig = plt.subplots(
        ...
        taxon_color = self.label_color  # real group -> colour map from __init__
        markers = markers or {}
        ...
        for name, (dfes, ne_dict, labels, color) in datasets.items():
            ...
            if color_by == "dataset":
                self.label_color = defaultdict(lambda c=color: c)
            ...
            for ax, stat in zip(axes[:len(self.stat_list)], self.stat_list):
                for pop, lab in zip(self.populations, self.labels):
                    ...
                    x = self.ne[pop]
                    pc = taxon_color[lab] if color_by == "taxon" else color

                    ax.scatter(x, m, color=pc, s=60, alpha=0.6, marker=marker)
                    ax.errorbar(
                        x, m,
                        yerr=[[max(lo, 0)], [max(hi, 0)]],
                        fmt="none", ecolor=pc, capsize=3, alpha=0.6
                    )

                reg_name = name if color_by == "taxon" else ""
                self._add_regression(ax, stat, color=color, name=reg_name, linestyle=ls)

            # one proxy handle per dataset (marker; plus line style in taxon mode)
            legend_handles[name] = plt.Line2D(
                [0], [0], marker=marker, linestyle=(ls if color_by == "taxon" else ""),
        ...
        # apply y-axis padding once after all datasets are drawn
        for ax in axes[:len(self.stat_list)]:
        ...
        axes[-1].axis("off")
        if color_by == "taxon":
            # taxonomy colour legend (groups): one row spanning the plot width,
            # with the dataset marker/line key placed below it.
            tax_handles = [
    ...
    def _plot_axes_stacked(self, fig, stat_list, stat_labels, show_legend, legend_n_cols=None) -> plt.Figure:
        ...
        if fig is None:
            fig = plt.subplots(
                len(stat_list) + 1,
                1,
                figsize=(10, 0.9 * len(stat_list) + 1),
                sharex=True,
                dpi=400
            )[0]
        axes = fig.axes
    ...
    def _add_regression(self, ax: Axes, stat: str, color="black", name: str = "", linestyle: str = "--"):
        """
        Add phylogenetic regression line to axis.

        ``name`` is an optional prefix (e.g. dataset name) used in the legend
        label; without it the label is just "r=..., p=...".
        """
        ...
        stats_label = f"r={r:.2f}, p={p:.3g}" if not np.isnan(p) else ''
        label = f"{name}: {stats_label}" if (name and stats_label) else stats_label

        ax.plot(np.exp(xx), yy, linestyle, color=color, label=label)
